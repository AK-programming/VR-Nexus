...
**The LLM is optional, and leaving it off is a supported configuration.** With `ANTHROPIC_API_KEY` empty, `llm_available` is false and every caller falls back: the parsers use their heading heuristics, `metadata.py` uses its label and keyword rules, and `rag.py` returns retrieved passages without generating prose over them. Parsing, chunking, embedding, search and duplicate detection are unaffected. What you lose is auto-tagging polish on documents whose structure the heuristics cannot resolve, and generated answers on `/api/library/ask`.

**One provider for the whole app: Anthropic Claude.** `app/services/library/llm.py` and `app/services/extraction.py` both talk to Claude through Anthropic's OpenAI-compatible endpoint (`ANTHROPIC_BASE_URL`, `ANTHROPIC_MODEL`), using the `openai` client purely as HTTP/SDK plumbing — the provider it's pointed at is always Anthropic, off the same `ANTHROPIC_API_KEY`. Library auto-tagging and the grounded Ask use `ANTHROPIC_MODEL` (the reasoning model); tender extraction uses the cheaper `ANTHROPIC_EXTRACTION_MODEL` for its high-volume, mechanical calls. Without a key, tender uploads fail at the extraction stage with a message naming that setting while everything else keeps working.

**One thing from the prototype is deliberately not reproduced.** Its config carried `OPENAI_USER_AGENT=claude-cli/1.0.60 (external, cli)` and its `llm.py` sent that as a `default_headers` override, with a comment explaining that the endpoint *"allowlists client User-Agents and rejects the SDK default."* That value is not configuration. Its only function was to make requests to a *third-party reseller* look like Anthropic's own first-party CLI so that reseller's allowlist would accept them — a problem that does not exist here, since this app talks to `api.anthropic.com` directly. It is omitted; `ANTHROPIC_USER_AGENT` exists as a setting only for the rare case of needing to override the SDK's own honest User-Agent, and is empty by default.
...
No API key is committed anywhere in this repository. `ANTHROPIC_API_KEY=` in `.env.example` is intentionally empty, and `.env` is gitignored and should stay that way.
