...
Retrieval (`services/search.py`) is Section 6's actual deliverable: it returns
the chunks, and Stage 3 requirement matching is the section that turns chunks
into prose. This module is the thin generation layer on top, so the library can
be demonstrated end to end without waiting for Stage 3.
...
from app.schemas import AnswerOut, SearchHit
