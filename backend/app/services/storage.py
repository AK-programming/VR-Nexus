...
def documents_dir() -> Path:
    path = Path(settings.STORAGE_DIR) / "documents"
...
def images_dir(document_id: uuid.UUID | str) -> Path:
    ...
    path = Path(settings.STORAGE_DIR) / "images" / str(document_id)
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def validate_upload(filename: str, content_type: str, size: int) -> str:
    ...
    if size > settings.max_upload_bytes:
        mb = size / 1024 / 1024
        raise UploadRejected(f"File is {mb:.1f} MB; the limit is {settings.MAX_UPLOAD_MB} MB.")
...
def delete_document_files(stored_path: str, document_id: uuid.UUID | str) -> None:
    ...
    img_dir = Path(settings.STORAGE_DIR) / "images" / str(document_id)
    if img_dir.exists():
...
def relative_path(path: Path | str) -> str:
    ...
    try:
        return str(Path(path).relative_to(Path(settings.STORAGE_DIR))).replace("\\", "/")
    except ValueError:
