...
class Page:
    ...
    # so the library survives a change of LIBRARY_STORAGE_DIR.
...
def get_parser(category: DocumentCategory) -> Callable[[RawDoc, bool], ParseResult]:
    ...
    Importing `app.services.library.parsers.registry` is what populates the
    table; a KeyError here means that import was skipped.
    """
    try:
        ...
        raise KeyError(
            ...
            "Import app.services.library.parsers.registry to load the built-in parsers."
        ) from None
