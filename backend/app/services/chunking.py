...
Deliberately separate from app/services/library/chunking.py, which chunks
Evidence Library assets into smaller section-bounded windows (~800 tokens, 100
overlap) and returns a different dataclass. Same idea, different tuning, and
different output type: this one carries page_start/page_end/section because a
tender requirement has to be traceable back to a page, while the library one
carries section_name/phase/image_paths because a case study is retrieved by its
structure. They are not interchangeable and neither should be folded into the
other.
