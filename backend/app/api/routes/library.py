...
def train(
    ...
    for document in documents:
        ...
        indexing.enqueue(document.id, job.id, use_llm=_settings.llm_available)
...
# ---------------------------------------------------------------------------
# Original file (the in-browser viewer)
# ---------------------------------------------------------------------------
...
def get_document_file(
    ...
    Added for the React viewer at /documents/:id/view, which renders PDFs with
    react-pdf and needs the bytes rather than the extracted text. Everything
    else in this router returns JSON, so nothing already here depended on it.

    Served inline rather than as an attachment: the point is to render it in the
    page, not to download it. The route is authenticated like every other one,
    so the client fetches it with its bearer token and hands the viewer a blob
    URL — an <iframe src> pointed straight at this path would arrive without the
    ...
    # file_path is written by storage.save_upload from a UUID, never from the
    # client's filename, so this can only fail if a row was hand-edited or the
    # storage directory moved. Checked anyway, because the alternative is
    # serving an arbitrary path off the host.
    if not resolved.is_relative_to(storage.documents_dir().resolve()):
        logger.warning("Refusing to serve %s: outside the library storage root", resolved)
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "File is outside the storage root")

    if not resolved.is_file():
        # The row exists but the bytes are gone — a restored database against an
        # empty storage volume, usually. Say so, rather than 500ing.
